...
exports.cargaMasiva = async (req, res) => {
  try {
    const datos = req.body; // Recibe los datos del cuerpo de la solicitud
    console.log('Datos recibidos para carga masiva:', datos);

    // Inserta todos los datos en la colección
    const result = await Datos.insertMany(datos);
    res.status(201).json({ message: 'Datos cargados exitosamente', data: result });
  } catch (error) {
    console.error('Error al cargar datos:', error);
    res.status(500).json({ message: 'Error al cargar datos', details: error.message });
  }
};
